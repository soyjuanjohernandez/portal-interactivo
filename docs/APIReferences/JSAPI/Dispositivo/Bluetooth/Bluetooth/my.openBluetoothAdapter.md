...
Utilice esta API para inicializar el módulo Bluetooth en el mini programa. Puede llamar a la siguiente API del miniprograma y recibir callbacks de eventos relacionados con el módulo Bluetooth en el periodo efectivo cuando se llama a la API ```my.openBluetoothAdapter```. El período efectivo finaliza cuando se llama a la API ```my.closeBluetoothAdapter```.

Instrucciones :

* Si llama a otras APIs en el módulo bluetooth antes de llamar a la API ```my.openBluetoothAdapter``` obtendrá el siguiente error:
    * Código de error: 10000
    * Descripción del error: El adaptador Bluetooth no está inicializado.
    * Solución: llame a la API ```my.openBluetoothAdapter```
  
* Cuando los usuarios no activan el Bluetooth o la función Bluetooth no es compatible con el teléfono móvil del usuario, se devuelve un error después de llamar a la API ```my.openBluetoothAdapter```. Para más información sobre los códigos de error, consulte la tabla de códigos de error. Una vez inicializado el módulo Bluetooth, puede utilizar la API [my.onBluetoothAdapterStateChange](./my.onBluetoothAdapterStateChange) para supervisar los cambios del estado de Bluetooth.
...
<table>
    ...
    <tr>
        ...
        <td>Booleano</td>
        <td>No</td>
        <td>Indica si desconectar automáticamente Bluetooth cuando deja la página actual. El valor por defecto es verdadero. Nota: Solo se admite Android.</td>
     </tr>
     <tr>
        <td>success</td>
        <td>Función</td>
...
<table>
    <tr>
     ...
     <tr>
        <td>isSupportBLE</td>
        <td>Booleano</td>
        <td>Indica si BLE es compatible.</td>
     </tr>
</table>
...
<table>
    <tr>
     ...
     <tr>
        <td>12</td>
        <td>Bluetooth no está encendido.</td>
        <td>Intente nuevamente encender Bluetooth.</td>
     </tr>
     <tr>
        <td>13</td>
        <td>La conexión al servicio del sistema se pierde temporalmente.</td>
        <td>Intente nuevamente volver a conectarse.</td>
     </tr>
     <tr>
        <td>14</td>
        <td>No autorizado para usar Bluetooth.</td>
        <td>Autorizar la aplicación para usar Bluetooth.</td>
     </tr>
     <tr>
        <td>15</td>
