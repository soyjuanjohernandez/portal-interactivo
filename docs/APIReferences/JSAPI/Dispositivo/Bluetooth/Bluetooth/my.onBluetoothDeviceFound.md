...
Utilice esta API cuando se encuentre un nuevo dispositivo Bluetooth.

**Instrucciones:**

Es posible que no obtenga el advertisData y RSSI en el emulador. Por favor, depure en el invitado.
Para el Entorno de Desarrollo Integrado (IDE) y dispositivos Android, el ID del dispositivo es la dirección MAC del dispositivo; para iOS devie, el ID del dispositivo es el UUID del dispositivo. Por lo tanto, no codifique el ID de dispositivo. Es necesario procesar el ID de dispositivo en diferentes plataformas; los dispositivos iOS se pueden emparejar dinámicamente basándose en propiedades como localName, advertisData y manufacturerData.
Si la API my.onBluetoothDeviceFound callback contiene un dispositivo bluetooth, el dispositivo se añade a la matriz obtenida por la API ```my.getBluetoothDevices```.
...
<table>
    <tr>
     ...
     <tr>
        <td>devices</td>
        <td>Array</td>
        <td>Una lista de todos los dispositivos recién descubiertos.</td>
     </tr>
</table>
